import React, { useContext, useState, useEffect } from 'react';
import { Context } from '../context/Context';
import '../styles/post.css';
import { FiUserPlus, FiUserMinus } from 'react-icons/fi';
import { FaHeart, FaRegHeart } from 'react-icons/fa';

const Post = ({ post }) => {
  const { user, likeOrUnlikePost, isFriend } = useContext(Context);
  const [isLiked, setIsLiked] = useState(false);

  // Check if the current user has liked the post
  useEffect(() => {
    let userLiked = post.likes[user.id] === true; // Check the Map for user's like status
    setIsLiked(userLiked);
  }, [post.likes, user.id]);

  const handleLikeClick = () => {
    likeOrUnlikePost(post._id); // Call the context method
    setIsLiked((prev) => !prev); // Optimistically update the state
  };

  return (
    <div className="post-card">
      <div className="user-header">
        <div className="user-info">
          <img
            src="data:image/png;base64,"
            alt={user.name}
          />
          <span>
            <h3>{user.name}</h3>
            <h4>India</h4>
          </span>
        </div>
        <div className="icons">
          {isFriend ? (
            <FiUserMinus className="add-user" />
          ) : (
            <FiUserPlus className="add-user" />
          )}
        </div>
      </div>
      <p className="post-content">{post.content}</p>
      <div className="post-footer">
        <div>
          <span onClick={handleLikeClick}>
            {isLiked ? <FaHeart className="liked" /> : <FaRegHeart />}
          </span>
          {post.likes.size} {/* Display the size of the Map as total likes */}
        </div>
        <p>Comments</p>
      </div>
    </div>
  );
};

export default Post;
